...
const CardPutToDeckModal = ({
  ...
  const explodeCard = useMemo(() => playerCards[uuid].find(item => cards[item].type === cardTypes.explodingKitten), [playerCards, cards, uuid]);
  const defuseCard = useMemo(() => playerCards[uuid].find(item => cards[item].type === cardTypes.defuse), [playerCards, cards, uuid]);
  ...
  const handleClick = () => {
    ...
    if (attackCount > 0) {
      updateDoc(doc(db, "game_rooms_kitten", id), {
        player_cards: {...playerCards, [uuid]: playerCards[uuid].filter(item => item !== defuseCard && item !== explodeCard)},
  ...
  function handleDrop({ e, card }) {
    ...
    if (dragEl === explodeCard) {
      ...
      const explodeCardIndex = newCardDeck.findIndex(item => item === explodeCard);
